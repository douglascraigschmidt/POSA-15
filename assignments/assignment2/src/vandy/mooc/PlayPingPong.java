 ...
 * @class PlayPingPong
 *
 * @brief This class uses elements of the Android HaMeR framework to create two Threads that alternately print "Ping"
 *        and "Pong", respectively, on the display.
 */
public class PlayPingPong implements Runnable {
	...
	/**
	 * Define a pair of Handlers used to send/handle Messages via the HandlerThreads.
	 */
	private final Handler[] handlers = new Handler[2];

	/**
	 * Define a CyclicBarrier synchronizer that ensures the HandlerThreads are fully initialized before the ping-pong
	 * algorithm begins.
	 */
	private final CyclicBarrier barrier = new CyclicBarrier(2);

	Handler getHandler(PingPong type) {
		return handlers[type.ordinal()];
	}

	/**
	 * Implements the concurrent ping/pong algorithm using a pair of Android Handlers (which are defined as an array
	 * field in the enclosing PlayPingPong class so they can be shared by the ping and pong objects). The class (1)
	 * extends the HandlerThread superclass to enable it to run in the background and (2) implements the
	 * Handler.Callback interface so its handleMessage() method can be dispatched without requiring additional
	 * subclassing.
	 */
	class PingPongThread extends HandlerThread implements Handler.Callback {
		 ...
		 */
		private final PingPong mMyType;
		...
		/**
		 * Constructor initializes the superclass and type field (which is either PING or PONG).
		 */
		...
		/**
		 * This hook method is dispatched after the HandlerThread has been started. It performs ping-pong initialization
		 * prior to the HandlerThread running its event loop.
		 */
		@Override
		...
		/**
		 * Hook method called back by HandlerThread to perform the ping-pong protocol concurrently.
		 */
		@Override
	...
	/**
	 * Start running the ping/pong code, which can be called from a main() method in a Java class, an Android Activity,
	 * etc.
	 */
	@Override
	public void run() {
